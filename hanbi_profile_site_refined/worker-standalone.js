  ...
  async fetch(request) {
    ...
    try {
      const icsRes = await fetch(ICS_URL, {
        headers: { "User-Agent": "Mozilla/5.0 HanbiCalendarWorker" },
        cf: { cacheTtl: 300, cacheEverything: true }
      });

      if (!icsRes.ok) return json({ ok: false, error: `ICS fetch failed: ${icsRes.status}` }, 502);

      const ics = await icsRes.text();
      if (!ics.includes("BEGIN:VCALENDAR")) return json({ ok: false, error: "Invalid ICS response" }, 502);
      ...
      return json({ ok: true, source: "google-calendar-ics", count: events.length, events }, 200);
    } catch (err) {
...
function parseDateField(field) {
  ...
  if (z === "Z") return new Date(Date.UTC(+yy, +mo - 1, +dd, +hh, +mi, +ss));
  return new Date(Date.UTC(+yy, +mo - 1, +dd, +hh - 9, +mi, +ss));
}

function parseRRule(value) {
  const out = {};
  value.split(";").forEach(part => { const [k, v] = part.split("="); out[k] = v; });
  return out;
}
...
function expandEvents(events, rangeStart, rangeEnd) {
  ...
  for (const ev of events) {
    ...
    result.push(...expandRRule(ev, start, duration, rangeStart, rangeEnd));
  }
  return result;
}
...
function expandRRule(ev, firstStart, duration, rangeStart, rangeEnd) {
  ...
  let cursor = new Date(firstStart), count = 0;
  while (cursor <= hardEnd && count < countLimit) {
    const instanceEnd = new Date(cursor.getTime() + duration);
    if (instanceEnd >= rangeStart && cursor <= rangeEnd) out.push(formatEvent(ev, cursor, instanceEnd));
    count++;
    if (freq === "DAILY") cursor = addDays(cursor, interval);
    else if (freq === "WEEKLY") cursor = addDays(cursor, interval * 7);
    else if (freq === "MONTHLY") cursor = addMonths(cursor, interval);
  }
  return out;
}

function parseUntil(v) { return parseDateField({ value: v, params: {} }) || new Date(Date.now() + 365 * 86400000); }
function addDays(d, days) { const n = new Date(d); n.setUTCDate(n.getUTCDate() + days); return n; }
function addMonths(d, months) { const n = new Date(d); n.setUTCMonth(n.getUTCMonth() + months); return n; }
function formatEvent(ev, start, end) {
  return { uid: ev.uid || "", summary: ev.summary || "제목 없음", description: ev.description || "", location: ev.location || "", start: start.toISOString(), end: end ? end.toISOString() : null };
}
